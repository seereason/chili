...
{-# language RankNTypes #-}
{- Apply some patches -}
module Dominator.PatchPartial where
...
-- This is a feature/optimization driven by alderon, where we want to allow native javascript widgets.
-- The 'protected node' in Alderon allows for persistent widgets to be attached.  But this causes
-- Dominator's diff to fail because it doesn't expect the extra nodes.  My original solution was
-- to detach the nodes, run the diff, and put them back, but that triggers focus events, which was
-- a crucial feature of the patch/diff method.
--
-- So this xxxPartial version recognizes leaves during the diff operation and prunes the traversal
-- of the children of the corresponding DOM node during the application of patches.
-- 
-- There remains the possibility that the patches will force recreation of the protected node, which
-- would leave the children abandoned. So it is the responsibility of the caller to ensure that
-- protected nodes are in stable positions and the children get cleaned up before the parent
-- node gets removed.
--CB
...
updateView isProtected (DHandle root vdom doc) newHtml =
  do oldHtml <- takeMVar vdom
     ...
     apply isProtected doc (toJSNode root) oldHtml patches
...
getNodes :: Debug => (Html -> Bool)
         ...
         -> IO [(Int, JSNode)]
getNodes isProtected currNode vdom nodeIndices = do
