...
foreign import javascript unsafe
  "($1,$2) => return ($1===$2)" js_eq :: JSVal  -> JSVal  -> Bool
...
foreign import javascript unsafe "($1) => return ($1 instanceof Element)"
...
foreign import javascript unsafe "($1) => alert($1)"
...
foreign import javascript unsafe "($1) => return ($1 instanceof Node)"
...
foreign import javascript unsafe "($1,$2) => $1[\"append\"]($2)"
...
foreign import javascript unsafe "($1) => return $1[\"children\"]"
...
foreign import javascript unsafe "($1) => return $1[\"length\"]" js_nodeListLength ::
        JSNodeList -> IO Int

nodeListLength :: (MonadIO m) => JSNodeList -> m Int
nodeListLength nodeList = liftIO $ js_nodeListLength nodeList

foreign import javascript unsafe "($1,$2) => $1[\"item\"]($2)" js_item ::
...
foreign import javascript unsafe "($1) => return $1[\"length\"]" js_length ::
        JSNode -> IO Word
...
foreign import javascript unsafe "($1,$2) => $1[\"contentEditable\"] = $2"
...
foreign import javascript unsafe "($1,$2) => return $1[\"compareDocumentPosition\"]($2)"
...
foreign import javascript unsafe "($1,$2) => return $1[\"contains\"]($2)"
...
foreign import javascript unsafe "($1,$2) => return $1[\"cloneNode\"]($2)"
...
foreign import javascript unsafe "($1) => return $1[\"nodeType\"]"
...
foreign import javascript unsafe "($1) => return $1[\"nodeName\"]"
...
foreign import javascript unsafe "($1) => return ($1 instanceof DocumentFragment)"
...
foreign import javascript unsafe "($1) => return ($1 instanceof Document)"
...
foreign import javascript unsafe "($1) => return ($1[\"implementation\"][\"createHTMLDocument\"]())"
...
foreign import javascript unsafe "($1) => document = $1"
...
foreign import javascript unsafe "($1) => return $1[\"body\"]"
...
foreign import javascript unsafe "($1,$2,$3,$4) => return $1[\"execCommand\"]($2,$3,$4)"
...
foreign import javascript unsafe "($1,$2) => return $1[\"queryCommandState\"]($2)"
...
foreign import javascript unsafe "($1) => return ($1 instanceof Window)"
...
foreign import javascript unsafe "($1) => window = $1"
...
foreign import javascript unsafe "($1) => return $1[\"scrollX\"]"
  js_scrollX :: JSWindow -> IO Double

scrollX :: (MonadIO m) => JSWindow -> m Double
scrollX w = liftIO (js_scrollX w)

foreign import javascript unsafe "($1) => return $1[\"scrollY\"]"
...
foreign import javascript unsafe "($1,$2) => $1[\"innerHTML\"] = $2"
...
foreign import javascript unsafe "($1) => $1[\"innerHTML\"]"
...
foreign import javascript unsafe "($1,$2) => $1[\"outerHTML\"] = $2"
...
foreign import javascript unsafe "($1) => return $1[\"tagName\"]"
...
foreign import javascript unsafe "($1,$2) => return $1[\"getElementsByClassName\"]($2)"
...
foreign import javascript unsafe "($1,$2,$3) => return ($1[\"insertAdjacentElement\"]($2, $3))"
...
foreign import javascript unsafe "($1,$2,$3) => return ($1[\"insertBefore\"]($2, $3))"
...
foreign import javascript unsafe "($1) => return $1[\"focus\"]()" js_focus :: JSElement -> IO ()

focus :: (MonadIO m) => JSElement -> m ()
focus e = liftIO (js_focus e)

foreign import javascript unsafe "($1) => $1[\"blur\"]()" js_blur :: JSElement -> IO ()
...
foreign import javascript unsafe "($1,$2) => $1[\"textContent\"] = $2"
...
foreign import javascript unsafe "($1,$2,$3,$4) => $1[\"replaceData\"]($2, $3, $4)" js_replaceData
    :: JSNode
    -> Word
    -> Word
    -> JSString
...
foreign import javascript unsafe "($1) => $1[\"remove\"]()"
        js_remove :: JSNode -> IO ()
...
foreign import javascript unsafe "($1) => return ($1[\"replaceChild\"]($2, $3))"
...
foreign import javascript unsafe "($1) => $1[\"replaceWith\"]($2)"
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"setAttribute\"]($2, $3)"
...
foreign import javascript unsafe "($1,$2) => $1[\"removeAttribute\"]($2)"
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"style\"][$2] = $3"
...
foreign import javascript unsafe "($1,$2) => delete $1[$2]"
...
foreign import javascript unsafe "($1) => return $1[\"checked\"]"
...
foreign import javascript unsafe "($1) => return $1[\"scrollTop\"]" js_scrollTop ::
        JSElement -> IO Double

foreign import javascript unsafe "($1) => return $1[\"scrollLeft\"]" js_scrollLeft ::
        JSElement -> IO Double
...
foreign import javascript unsafe "($1,$2,$3,$4) => $1[\"scrollTo\"]({top: $2, left: $3, behavior: $4})" js_scrollTo ::
...
foreign import javascript unsafe "($1,$2,$3,$4) => $1[\"scrollBy\"]({top: $2, left: $3, behavior: $4})" js_scrollBy ::
    JSVal -> Double -> Double -> JSString -> IO ()
...
foreign import javascript unsafe "($1) => return $1[\"value\"]"
...
foreign import javascript unsafe "($1,$2) => $1[\"value\"] = $2"
...
foreign import javascript unsafe "($1) => return $1[\"hasFocus\"]()"
        js_hasFocus :: JSDocument -> IO Bool

hasFocus :: (MonadIO m) => JSDocument -> m Bool
hasFocus doc = liftIO (js_hasFocus doc)

foreign import javascript unsafe "($1,$2) => return $1[\"matches\"]($2)"
...
foreign import javascript unsafe "($1,$2) => return ($1[\"dataset\"][$2])"
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"dataset\"][$2] = $3"
...
foreign import javascript unsafe "($1,$2,$3) => return ($1[\"attachShadow\"]({mode: $2, delegatesFocus: $3}))"
...
foreign import javascript unsafe "($1) => return ($1 instanceof Text)"
...
foreign import javascript unsafe "($1,$2) => return $1[\"isEqualNode\"]($2)"
...
foreign import javascript unsafe "($1) => return $1[\"length\"]"
...
foreign import javascript unsafe "($1,$2) => $1[\"nodeValue\"] = $2"
...
foreign import javascript unsafe "($1) => return $1[\"data\"]" js_inputData ::
...
foreign import javascript unsafe "($1) => return $1[\"dataTransfer\"]" js_inputDataTransfer ::
...
foreign import javascript unsafe "($1) => return $1[\"inputType\"]" inputType ::
        InputEventObject ev -> JSString

foreign import javascript unsafe "($1) => return $1[\"isComposing\"]" isComposing ::
        InputEventObject ev -> Bool

foreign import javascript unsafe "($1) => return ($1[\"getTargetRanges\"]())" js_getTargetRanges ::
...
foreign import javascript unsafe "($1) => return $1[\"defaultPrevented\"]" js_defaultPrevented ::
...
foreign import javascript unsafe "($1) => return $1[\"currentTarget\"]" js_currentTarget ::
...
foreign import javascript unsafe "($1) => returb $1[\"target\"]" js_target ::
        EventObject ev -> EventTarget

target :: (IsEventObject obj) => obj -> EventTarget
target obj = js_target (asEventObject obj)

foreign import javascript unsafe "($1) => $1[\"preventDefault\"]()" js_preventDefault ::
        EventObject ev -> IO ()

preventDefault :: (IsEventObject obj) => obj -> IO ()
preventDefault obj = (js_preventDefault (asEventObject obj))

foreign import javascript unsafe "($1) => $1[\"stopPropagation\"]()" js_stopPropagation ::
        EventObject ev -> IO ()
...
foreign import javascript unsafe "($1) => return $1[\"clientX\"]" clientX ::
        MouseEventObject ev -> Double

foreign import javascript unsafe "($1) => return $1[\"clientY\"]" clientY ::
        MouseEventObject ev -> Double

foreign import javascript unsafe "($1) => return $1[\"button\"]" button ::
        MouseEventObject ev -> Int

foreign import javascript unsafe "($1) => return $1[\"shiftKey\"]" mouse_shiftKey ::
        MouseEventObject ev -> Bool

foreign import javascript unsafe "($1) => return $1[\"ctrlKey\"]" mouse_ctrlKey ::
        MouseEventObject ev -> Bool

foreign import javascript unsafe "($1) => return $1[\"altKey\"]" mouse_altKey ::
        MouseEventObject ev -> Bool

foreign import javascript unsafe "($1) => return $1[\"metaKey\"]" mouse_metaKey ::
        MouseEventObject ev -> Bool
...
foreign import javascript unsafe "($1) => return $1[\"charCode\"]" charCode ::
        (KeyboardEventObject ev) -> Int

foreign import javascript unsafe "($1) => return $1[\"keyCode\"]" keyCode ::
        (KeyboardEventObject ev) -> Int

foreign import javascript unsafe "($1) => return $1[\"which\"]" which ::
        (KeyboardEventObject ev) -> Int

foreign import javascript unsafe "($1) => return $1[\"repeat\"]" repeat ::
...
foreign import javascript unsafe "($1) => return $1[\"shiftKey\"]" keyboard_shiftKey ::
        (KeyboardEventObject ev) -> Bool

foreign import javascript unsafe "($1) => return $1[\"ctrlKey\"]" keyboard_ctrlKey ::
        (KeyboardEventObject ev) -> Bool

foreign import javascript unsafe "($1) => return $1[\"altKey\"]" keyboard_altKey ::
        (KeyboardEventObject ev) -> Bool

foreign import javascript unsafe "($1) => return $1[\"metaKey\"]" keyboard_metaKey ::
        (KeyboardEventObject ev) -> Bool
...
foreign import javascript unsafe "($1) => return $1[\"relatedTarget\"]" js_relatedTarget ::
...
foreign import javascript unsafe "($1,$2,$3,$4) => return (new CustomEvent($1, { 'detail': $2, 'bubbles' : $3, 'cancelable' : $4}))"
...
foreign import javascript unsafe "($1) => return $1[\"detail\"]"
...
foreign import javascript unsafe "($1,$2,$3,$4) => $1[\"addEventListener\"]($2, $3,\n$4)"
...
-- FIXME: Element is overly restrictive
foreign import javascript unsafe "($1,$2,$3,$4,$5,$6) => $1['addEventListener']($2, $3,{'capture':$4,'once':$5,'passive':$6})"
...
foreign import javascript unsafe "($1,$2) => return $1[\"dispatchEvent\"]($2)"
...
foreign import javascript unsafe "($1) => return $1[\"top\"]" rectTop ::
         DOMClientRect -> Double

foreign import javascript unsafe "($1) => return $1[\"left\"]" rectLeft ::
         DOMClientRect -> Double

foreign import javascript unsafe "($1) => return $1[\"right\"]" rectRight ::
         DOMClientRect -> Double

foreign import javascript unsafe "($1) => return $1[\"bottom\"]" rectBottom ::
         DOMClientRect -> Double

foreign import javascript unsafe "($1) => return $1[\"height\"]" height ::
         DOMClientRect -> Double

foreign import javascript unsafe "($1) => return $1[\"getBoundingClientRect\"]()" js_getBoundingClientRect ::
...
foreign import javascript unsafe "($1) => return $1[\"offsetWidth\"]" js_offsetWidth ::
  JSElement -> IO Int
...
foreign import javascript unsafe "($1,$2,$3) => return $3.slice($1, $1 + $2)"
...
foreign import javascript unsafe "($1) => return ($1 instanceof XMLHttpRequest)"
...
foreign import javascript unsafe "($1,$2,$3,$4) => $1[\"open\"]($2, $3, $4)"
        js_open ::
        XMLHttpRequest ->
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"setRequestHeader\"]($2,$3)"
        js_setRequestHeader
            :: XMLHttpRequest
            -> JSString
            -> JSString
...
foreign import javascript unsafe "($1) => $1[\"send\"]()" js_send ::
        XMLHttpRequest -> IO ()
...
foreign import javascript unsafe "($1,$2) => $1[\"send\"]($2)" js_sendString ::
...
foreign import javascript unsafe "($1,$2) => $1[\"send\"]($2)" js_sendArrayBuffer ::
...
-}
foreign import javascript unsafe "($1,$2) => $1[\"send\"]($2)" js_sendData ::
        XMLHttpRequest
    -> JSVal
    -> IO ()
...
foreign import javascript unsafe "($1,$2) => $1[\"responseType\"] = $2"
        js_setResponseType ::
...
foreign import javascript unsafe "($1) => return $1[\"responseText\"]"
...
foreign import javascript unsafe "($1) => return $1[\"dataTransfer\"]" js_dataTransfer ::
...
foreign import javascript unsafe "($1,$2) => $1[\"addRange\"]($2)"
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"collapse\"]($2, $3)"
...
foreign import javascript unsafe "($1) => return $1[\"isCollapsed\"]"
  js_isCollapsed :: Selection -> IO Bool

isCollapsed :: (MonadIO m) => Selection -> m Bool
isCollapsed sel = liftIO $ js_isCollapsed sel

foreign import javascript unsafe "($1,$2) => return $1[\"getRangeAt\"]($2)"
...
foreign import javascript unsafe "($1) => $1[\"removeAllRanges\"]()"
...
foreign import javascript unsafe "($1,$2, $3, $4, $5) => $1[\"setBaseAndExtent\"]($2, $3, $4, $5)"
...
foreign import javascript unsafe "($1) => return $1[\"toString\"]()"
...
foreign import javascript unsafe "($1,$2,$3) => return $1[\"containsNode\"]($2,$3)"
...
foreign import javascript unsafe "($1) => return $1[\"cloneContents\"]()"
...
foreign import javascript unsafe "($1) => $1[\"deleteContents\"]()"
  js_deleteContents :: Range -> IO ()

deleteContents :: (MonadIO m) => Range -> m ()
deleteContents r = liftIO $ js_deleteContents r

foreign import javascript unsafe "($1) => $1[\"getBoundingClientRect\"]()" js_getRangeBoundingClientRect ::
...
-}
foreign import javascript unsafe "($1,$2) => $1[\"selectNode\"]($2)"
...
foreign import javascript unsafe "($1,$2) => $1[\"selectNodeContents\"]($2)"
...
foreign import javascript unsafe "($1,$2) => $1[\"insertNode\"]($2)"
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"setStart\"]($2,$3)"
...
foreign import javascript unsafe "($1,$2) => $1[\"setStartBefore\"]($2)"
...
foreign import javascript unsafe "($1,$2) => $1[\"setStartAfter\"]($2)"
...
foreign import javascript unsafe "($1,$2) => $1[\"setEndAfter\"]($2)"
...
foreign import javascript unsafe "($1,$2) => return $1[\"item\"]($2)" js_collectionItem ::
...
foreign import javascript unsafe "($1) => return $1[\"length\"]" js_collectionLength ::
...
foreign import javascript unsafe "($1,$2) => return $1[\"getData\"]($2)" js_getDataTransferData ::
...
foreign import javascript unsafe "($1,$2,$3) => $1[\"setData\"]($2, $3)" js_setDataTransferData ::
...
foreign import javascript unsafe "($1) => return $1[\"kind\"]()" js_dataTransferItemKind ::
...
foreign import javascript unsafe "($1) => return $1[\"type\"]()" js_dataTransferItemType ::
        DataTransferItem -> JSString
...
foreign import javascript unsafe "($1) => return $1[\"clipboardData\"]" clipboardData ::
        ClipboardEventObject ev -> IO DataTransfer

-- * Event

foreign import javascript unsafe "($1,$2,$3) => return (new Event($1, { 'bubbles' : $2, 'cancelable' : $3}))"
...
foreign import javascript unsafe "($1,$2) => return (new $1.JSDOM($2).window)"
...
foreign import javascript unsafe "($1,$2) => $1[\"add\"]($2)"
...
foreign import javascript unsafe "($1,$2) => $1[\"remove\"]($2)"
...
foreign import javascript unsafe "($1,$2) => return $1[\"replace\"]($2)"
...
foreign import javascript unsafe "($1) => return $1[\"data\"]"
...
foreign import javascript unsafe "($1) => return $1[\"currentScript\"]" js_currentScript ::
  JSDocument -> IO JSVal
...
foreign import javascript unsafe "($1) => return $1[\"caretPositionFromPoint\"]($2,$3)"
  caretPositionFromPoint :: JSDocument -> Double -> Double -> IO CaretPos

foreign import javascript unsafe "($1) => return $1[\"offsetNode\"]"
  offsetNode :: CaretPos -> JSNode

foreign import javascript unsafe "($1) => return $1[\"offset\"]"
  offset :: CaretPos -> Int
...
foreign import javascript unsafe "($1,$2,$3) => return $1[\"caretRangeFromPoint\"]($2,$3)"
